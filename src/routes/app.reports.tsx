...
function ReportsPage() {
  ...
  const exportCsv = (label: string) => {
    const rows = [
      ...
      ...goals.map((g) => [userById(g.ownerId)?.name ?? "", g.title, g.status, g.target, g.weightage, scoreForGoal(g)]),
    ];
    const csv = rows.map((r) => r.map((c) => `"${String(c).replace(/"/g, '""')}"`).join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = `intragoals-${label}.csv`; a.click();
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <div className="text-sm text-muted-foreground">Export CSV / Excel snapshots of your cycle</div>
        </div>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {[
          ...
          <div key={r} className="rounded-2xl glass p-5 flex items-center justify-between">
            ...
            <Button size="sm" variant="outline" className="border-border" onClick={() => exportCsv(r)}>
