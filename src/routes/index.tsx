  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border/70 bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-[82px] max-w-[1390px] items-center justify-between gap-4 px-5 md:px-8 xl:px-12">
        ...
        <Button asChild className="hidden h-12 rounded-lg bg-brand-gradient px-6 text-sm font-bold tracking-wide text-white shadow-lg shadow-cyan-500/20 hover:opacity-95 sm:inline-flex lg:px-7">
...
function Hero() {
  return (
    <section id="home" className="relative min-h-[610px] overflow-hidden bg-[#f7f9ff] sm:min-h-[650px] md:min-h-[660px]">
      ...
      <div className="animate-slide-in-soft relative z-10 mx-auto max-w-7xl px-5 pb-20 pt-10 text-center sm:pt-12 md:pt-16">
        <h1 className="hero-title logo-ink mx-auto max-w-7xl">
          <span className="text-slate-400">Stop Chasing.</span> <span className="text-[color:var(--brand-ink)]">Start</span>
          <br />
        ...
        <div className="mt-7 flex flex-wrap items-center justify-center gap-3 sm:gap-5">
          <Button asChild size="lg" variant="outline" className="h-14 rounded-lg border-[color:var(--brand-cyan)]/30 bg-white px-5 text-sm font-extrabold text-[color:var(--brand-ink)] shadow-sm hover:border-[color:var(--brand-purple)]/35 hover:bg-cyan-50/40 sm:h-[60px] sm:px-8 sm:text-base">
            <Link to="/app/dashboard">VIEW DASHBOARD</Link>
          </Button>
          <Button asChild size="lg" className="h-14 rounded-lg bg-brand-gradient px-6 text-sm font-extrabold text-white shadow-xl shadow-cyan-500/20 hover:opacity-95 sm:h-[60px] sm:px-9 sm:text-base">
            <Link to="/login">GET STARTED</Link>
          </Button>
        </div>
      </div>
...
function HeroMockups() {
  return (
    <div className="hero-visual-stage pointer-events-none absolute inset-x-0 bottom-[-260px] z-0 mx-auto h-[460px] max-w-7xl sm:bottom-[-260px] md:bottom-[-250px]">
      <div className="animate-float-slow absolute -left-6 bottom-8 hidden md:block" style={{ "--mockup-rotate": "-22deg" } as Record<string, string>}>
        <IdeaCard />
      </div>
      <div className="animate-float-medium absolute bottom-8 left-[44%] hidden w-[360px] opacity-95 sm:block md:w-[470px]" style={{ "--mockup-rotate": "-24deg", "--mockup-x": "-50%" } as Record<string, string>}>
        <DesktopMockup accent="blue" title="Goal Command Center" />
      </div>
      <div className="animate-float-slow absolute bottom-10 right-[-105px] w-[500px] opacity-90 sm:right-[-70px] sm:w-[560px] md:right-[-50px] md:w-[690px] md:opacity-95" style={{ "--mockup-rotate": "-25deg" } as Record<string, string>}>
        <DesktopMockup accent="orange" title="Manager Approval Queue" />
      </div>
      <div className="animate-float-medium absolute bottom-0 right-[26%] hidden w-[230px] lg:block" style={{ "--mockup-rotate": "-24deg" } as Record<string, string>}>
        <PhoneMockup />
      </div>
    </div>
  );
      ...
      <div className="mx-auto max-w-7xl px-5">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {services.map((service) => (
            <div key={service.title} className="rounded-lg border border-slate-200 bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-200/70">
              <div className="grid h-14 w-14 place-items-center rounded-lg bg-cyan-50 text-[color:var(--brand-purple)]">
                <service.icon className="h-7 w-7" />
              </div>
              <h3 className="mt-7 text-xl font-black text-[color:var(--brand-ink)]">{service.title}</h3>
        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {stats.map((stat) => (
            <div key={stat.label} className="rounded-lg border border-slate-200 bg-white p-6 text-center shadow-sm">
              ...
              <div className="mt-5 text-4xl font-black text-[color:var(--brand-ink)]">{stat.value}</div>
...
function HowItWorks() {
  const steps = [
    { n: "01", icon: Workflow, title: "PLAN", text: "HR opens the cycle and employees draft structured goals with weighted outcomes." },
    { n: "02", icon: CheckCircle2, title: "APPROVE", text: "Managers review submissions, request changes, and lock approved goals." },
    { n: "03", icon: CalendarCheck, title: "CHECK IN", text: "Teams update quarterly progress with planned versus actual achievement." },
    { n: "04", icon: FileBarChart, title: "REPORT", text: "Leadership exports progress, audit logs, and cycle health without manual cleanup." },
  ];
  return (
    <section className="bg-white py-24">
      <div className="mx-auto max-w-7xl px-5">
        ...
        <div className="mt-14 grid gap-6 md:grid-cols-4">
          {steps.map((step) => (
            <div key={step.n} className="relative rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <h3 className="mt-6 text-xl font-black text-[color:var(--brand-ink)]">{step.title}</h3>
            ...
            >
              ...
              >
                <div className={index % 2 ? "min-[560px]:order-2" : ""}>
                  ...
                  <h3 className="text-2xl font-black leading-tight text-[color:var(--brand-ink)]">{project.title}</h3>
                  ...
                  <Button asChild className="mt-7 h-11 rounded-lg bg-brand-gradient px-5 text-white shadow-lg shadow-cyan-500/15 hover:opacity-95">
                    <Link to="/login">VIEW DEMO <ArrowRight className="ml-2 h-4 w-4" /></Link>
                  </Button>
                </div>
    ...
    >
      <div className="mockup-screen h-full rounded-2xl bg-white p-4">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">{screen.cycle}</div>
            <div className="mt-1 text-sm font-black text-[color:var(--brand-ink)]">{screen.title}</div>
          </div>
          <div className="grid h-11 w-11 place-items-center rounded-xl text-white shadow-lg shadow-cyan-500/20" style={{ background: color }}>
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  <div className="truncate text-xs font-black text-[color:var(--brand-ink)]">{row.label}</div>
                </div>
                <div className="ml-2 rounded-full bg-white px-2 py-1 text-[10px] font-black text-slate-500 shadow-sm">{row.status}</div>
              </div>
              <div className="mt-3 h-2 rounded-full bg-slate-200">
                <div className="progress-flow progress-fill h-full rounded-full" style={{ width: `${row.width}%`, background: color }} />
              </div>
  ...
  return (
    <section className="bg-white py-24">
      <div className="mx-auto max-w-7xl px-5">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {quotes.map((item) => (
            <div key={item.name} className="rounded-lg border border-slate-200 bg-white p-7 shadow-sm">
              ...
              <div className="mt-7 flex items-center justify-between">
                ...
                <div className="text-brand-gradient rounded-lg bg-cyan-50 px-3 py-2 text-sm font-black">{item.score}</div>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <section id="contact" className="bg-[#f7f9ff] py-24">
      <div className="mx-auto grid max-w-7xl gap-10 px-5 md:grid-cols-[1.2fr_0.8fr] md:items-center">
        <div>
          <div className="text-brand-gradient text-sm font-black uppercase tracking-[0.22em]">Contact Us</div>
          ...
          <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-600">
            Start with a demo, explore employee, manager, and admin views, then shape the workflow for your organization.
          </p>
          <Button asChild size="lg" className="mt-8 h-14 rounded-lg bg-brand-gradient px-8 text-base font-extrabold text-white shadow-xl shadow-cyan-500/20">
            <Link to="/login">GET STARTED</Link>
          </Button>
        </div>
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-5 py-12 md:grid-cols-4">
        <div>
          ...
          <p className="mt-4 max-w-xs text-sm leading-6 text-slate-600">
            Enterprise goal setting and tracking for modern teams that need clarity, accountability, and speed.
          </p>
        </div>
        <FooterCol title="Product" links={[["Home", "/"], ["Features", "/features"], ["Pricing", "/pricing"]]} />
        <FooterCol title="Workspace" links={[["Dashboard", "/app/dashboard"], ["Login", "/login"], ["Register", "/register"]]} />
...
function FooterCol({ title, links }: { title: string; links: [string, string][] }) {
  return (
    <div>
      <div className="text-sm font-black uppercase tracking-[0.18em] text-[color:var(--brand-ink)]">{title}</div>
      <ul className="mt-4 space-y-2 text-sm text-slate-600">
        {links.map(([label, to]) => (
          <li key={to}>
            <Link to={to} className="nav-gradient-link">{label}</Link>
          </li>
        ))}
      </ul>
    </div>
...
function SectionHeader({ eyebrow, title, text }: { eyebrow: string; title: string; text: string }) {
  return (
    <div className="mx-auto max-w-3xl text-center">
      <div className="text-brand-gradient text-sm font-black uppercase tracking-[0.22em]">{eyebrow}</div>
      <h2 className="display-font mt-4 text-4xl text-[color:var(--brand-ink)] md:text-5xl">{title}</h2>
