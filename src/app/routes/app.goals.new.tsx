  ...
  const persist = async (status: "Draft" | "Pending Approval") => {
    if (status === "Pending Approval") {
      ...
      if (items.some((i) => !i.title.trim()))
        return toast.error("Empty goal titles are not allowed.");
    }
    setSaving(status);
    try {
      for (const it of items) {
        ...
        if (authSource === "account") {
          ...
          if (status === "Pending Approval") {
            await workspaceApi.createGoalApprovalNotifications(goal.id, goal.title, user.name);
          }
        }
      }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <div className="text-sm text-muted-foreground">
            FY26 cycle � up to 8 goals � weightages must total 100%
          </div>
        </div>
        <div className="flex gap-2">
          ...
          >
            <Save className="mr-1.5 h-4 w-4" /> {saving === "Draft" ? "Saving..." : "Save draft"}
          </Button>
          <Button
            className="bg-brand-gradient text-background hover:opacity-90"
            disabled={Boolean(saving)}
            onClick={() => void persist("Pending Approval")}
          >
            <Send className="mr-1.5 h-4 w-4" />{" "}
            {saving === "Pending Approval" ? "Submitting..." : "Submit for approval"}
          </Button>
        </div>
      </div>
      ...
      <div className="rounded-2xl glass p-4">
        <div className="flex items-center justify-between text-xs">
          <div className="text-muted-foreground">
            Goals: <span className="text-foreground">{items.length} / 8</span>
          </div>
          <div
            className={
              totalWeight === 100
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              <div className="md:col-span-2 space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="e.g. Reduce P95 API latency by 30%"
                />
              </div>
              <div className="md:col-span-2 space-y-1.5">
                ...
                <Textarea
                  ...
                  placeholder="What does success look like? What is the approach?"
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Select
                  value={it.thrustArea}
                  onValueChange={(v) => update(it.id, { thrustArea: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={it.direction}
                  onValueChange={(v: ScoreDirection) => update(it.id, { direction: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    max={100}
                    value={it.weightage}
                    onChange={(e) => update(it.id, { weightage: Number(e.target.value) })}
                  />
                </div>
              </div>
            </div>
