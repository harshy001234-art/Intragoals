...
import { KpiCard } from "@/components/shared/goal-primitives";
...
function AdminDash() {
  ...
  const departments = Array.from(new Set(people.map((person) => person.department))).filter(
    Boolean,
  );
  const deptData = departments.map((department) => {
    const departmentUsers = people.filter((person) => person.department === department);
    const departmentGoals = goals.filter((goal) =>
      departmentUsers.some((person) => person.id === goal.ownerId),
    );
    const score = departmentGoals.length
      ? Math.round(
          departmentGoals.reduce((sum, goal) => sum + scoreForGoal(goal), 0) /
  ...
  const statusData = [
    ...
    {
      name: "Pending",
      value: goals.filter((g) => g.status === "Pending Approval").length,
      color: "#ffd43d",
    },
    { name: "Draft", value: goals.filter((g) => g.status === "Draft").length, color: "#94A3B8" },
    {
      name: "Returned",
      value: goals.filter((g) => g.status === "Returned").length,
      color: "#ff7a18",
    },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <KpiCard
          ...
          value={`${completion}%`}
          hint={`${approved} / ${totalGoals} goals approved`}
          accent="blue"
        />
        <KpiCard
          label="Active Escalations"
          value={escalations.filter((e) => !e.resolved).length}
          hint="Across L1 and HR levels"
          accent="orange"
        />
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-2xl glass p-5">
          <div className="flex items-center justify-between">
            ...
            <Link to="/app/audit" className="text-xs text-muted-foreground hover:text-foreground">
              View all
            </Link>
          </div>
