import { type Goal, type GoalStatus } from "@/lib/store";
import { cn } from "@/lib/utils";

const STATUS_STYLES: Record<GoalStatus, string> = {
  Draft: "bg-secondary text-muted-foreground border-border",
  "Pending Approval": "bg-[color:var(--brand-yellow)]/10 text-[color:var(--brand-yellow)] border-[color:var(--brand-yellow)]/30",
  Approved: "bg-[color:var(--brand-green)]/10 text-[color:var(--brand-green)] border-[color:var(--brand-green)]/30",
  Rejected: "bg-destructive/15 text-destructive border-destructive/30",
  Returned: "bg-[color:var(--brand-orange)]/10 text-[color:var(--brand-orange)] border-[color:var(--brand-orange)]/30",
  Locked: "bg-[color:var(--brand-purple)]/10 text-[color:var(--brand-purple)] border-[color:var(--brand-purple)]/30",
};

export function StatusBadge({ status }: { status: GoalStatus }) {
  return (
    <span className={cn("inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider", STATUS_STYLES[status])}>
      {status}
    </span>
  );
}

export function ProgressRing({ value, size = 64, stroke = 7, label }: { value: number; size?: number; stroke?: number; label?: string }) {
...
export function KpiCard({
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl glass p-5">
      <div className={cn("pointer-events-none absolute -right-10 -top-10 h-32 w-32 rounded-full blur-2xl bg-gradient-to-br", accentMap[accent])} />
